...
const CATALOG = {
  ...
  surgery:{name:'手术包',   type:'surgery',  value:400,  price:400},
  ...
  keycard_green: {name:'绿卡(密室钥匙)', type:'keycard', value:0, price:400, tier:'green'},
  keycard_blue:  {name:'蓝卡(密室钥匙)', type:'keycard', value:0, price:900, tier:'blue'},
  keycard_purple:{name:'紫卡(密室钥匙)', type:'keycard', value:0, price:1800, tier:'purple'},
  keycard_gold:  {name:'金卡(密室钥匙)', type:'keycard', value:0, price:3600, tier:'gold'}
};
const TYPE_CN = {weapon:'武器', armor:'护甲', med:'医疗', surgery:'手术', ammo:'弹药', valuable:'物资', keycard:'房卡'};
...
function defaultProfile(){
  return {
    money: 3000,
    stash: { rifle:1, armor2:1, med:2, surgery:1, ammo:1, keycard_green:1 },
    equipped: { weapon:'rifle', weapon2:'pistol', armorLevel:2, meds:2, ammo:1, surgery:1, role:'assault',
                keycards:{green:1,blue:0,purple:0,gold:0}, insure:false,
...
function add(id,n){ const v = count(id) + (n||1); Profile.stash[id] = (id && id.indexOf('keycard')===0) ? Math.min(v, 9) : Math.min(v, 99999); }
...
function normalize(){
  ...
  e.surgery = clamp(e.surgery, 0, Math.min(5, count('surgery')));
  e.keycards = e.keycards||{green:0,blue:0,purple:0,gold:0};
  for(const t of ['green','blue','purple','gold']) e.keycards[t]=clamp(e.keycards[t]||0,0,Math.min(5, count('keycard_'+t)));
  if(!window.ROLES || !window.ROLES[e.role]) e.role='assault';
}
...
function setRole(id){ if(window.ROLES && window.ROLES[id]){ Profile.equipped.role=id; save(); render(); } }
function setTier(k){ if(OPS_TIERS[k]){ Profile.equipped.tier=k; save(); render(); } }
function setField(k,v){ Profile.equipped[k]=v; save(); }
function toggleMp(){
  Profile.equipped.mp=!Profile.equipped.mp; save();
  if(Profile.equipped.mp){ if(window.Net && Net.Lobby && !Net.Lobby.isOpen) Net.Lobby.open(onLobbyList); }
  else { if(window.Net && Net.Lobby) Net.Lobby.close(); }
  render();
}
function genRoom(){ Profile.equipped.room='R'+Math.floor(Math.random()*900000+100000); save(); render(); }

let lobbyRooms=[];
function onLobbyList(arr){ lobbyRooms=arr||[]; refreshLobbyUI(); }
function refreshLobbyUI(){
  const el=document.getElementById('lobbyList'); if(!el) return;
  if(!lobbyRooms.length){ el.innerHTML='<div class="meta">暂无在线房间。点击「创建房间」开局，或在下方填写房间码加入。</div>'; return; }
  let s='';
  for(const r of lobbyRooms){
    const m=(OPS_MAPS.find(x=>x.id===r.map)||{}).name||r.map||'随机';
    const t=(OPS_TIERS[r.tier]||OPS_TIERS.normal||{}).name||r.tier||'常规';
    s+=`<div class="item"><div><span class="nm">房间 ${r.room}</span> <span class="meta">${m} · ${t} · ${(r.count||1)}人在线 · 房主 ${r.host||'未知'}</span></div>
      <button onclick="Meta.joinRoom('${r.room}')">加入</button></div>`;
  }
  el.innerHTML=s;
}
function createRoom(){
  ...
  setField('room', code);
  flash('已创建房间 '+code+'，进入等待室');
  enterRoom(true);
}
function joinRoom(room){
  room = room || Profile.equipped.room;
  if(!room){ flash('请选择房间或填写房间码'); return; }
  setField('room', room);
  enterRoom(false);
}
...
function step(kind, d){
  ...
  if(kind==='surgery') e.surgery = clamp(e.surgery+d, 0, Math.min(5, count('surgery')));
  save(); render();
}
...
// 房卡自动携带（携带全部持有，上限 5/档）
function carryKeycards(){
  const o={green:0,blue:0,purple:0,gold:0};
  for(const t of ['green','blue','purple','gold']) o[t]=Math.min(5, count('keycard_'+t));
  return o;
}
function deploy(){
  normalize();
  const e = Profile.equipped;
  const cfg = {
    weapon:e.weapon, weapon2:e.weapon2, armorLevel:e.armorLevel, meds:e.meds, ammo:e.ammo, surgery:e.surgery,
    keycards: carryKeycards(),
    insure:e.insure, map:e.map, tier:e.tier, mp:e.mp,
    name:(e.name||'玩家').slice(0,12), room:e.room
  };
  if(e.mp) enterRoom(true);
  else consumeAndStart(cfg);
}

// 真正进入战场：扣除保险金与消耗品，构建快照，启动游戏
function consumeAndStart(cfg){
  normalize();
  const e=Profile.equipped;
  if(cfg.insure){
    const cost=insCost();
  ...
  if(e.surgery>0) remove('surgery', e.surgery);
  for(const t of ['green','blue','purple','gold']){ const n=Math.min(5,count('keycard_'+t)); if(n>0) remove('keycard_'+t, n); }
  deploySnapshot = JSON.parse(JSON.stringify(e));
  save(); hideBase(); hideWaitRoom();
  window.startGame(cfg);
}

/* ---------------- 联机房间等待室 ---------------- */
let roomConfig=null, waitEl=null, waitTimer=null, hostName='';
function enterRoom(host){
  normalize();
  hideBase();
  showWaitRoom();
  const e=Profile.equipped;
  roomConfig=null; hostName='';
  if(window.Net && Net.connect){
    Net.connect({
      room: e.room || ('R'+Math.floor(Math.random()*900000+100000)),
      name: (e.name||'玩家').slice(0,12), map: e.map, tier: e.tier, host: !!host,
      onReady:(info)=>{ roomConfig=(info&&info.config)||null; if(info&&info.host) hostName=info.host; renderWaitRoom(); },
      onStatus:(s)=>{ mpStatus=s; flash(s); },
      onError:()=>{ flash('房间服务器未连接，单机模式'); hideWaitRoom(); showBase(); }
    });
  } else {
    flash('联机模块未加载'); showBase();
  }
}
function showWaitRoom(){
  if(!waitEl){
    waitEl=document.createElement('div'); waitEl.id='waitroom';
    waitEl.style.cssText='position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:radial-gradient(circle at 50% 30%,#16202b,#070b0f);z-index:11;font-family:"Consolas","Courier New",monospace;';
    document.body.appendChild(waitEl);
  }
  waitEl.style.display='flex';
  renderWaitRoom();
  if(waitTimer) clearInterval(waitTimer);
  waitTimer=setInterval(()=>{ if(waitEl && waitEl.style.display!=='none') renderWaitRoom(); }, 1000);
}
function hideWaitRoom(){ if(waitEl) waitEl.style.display='none'; if(waitTimer){ clearInterval(waitTimer); waitTimer=null; } }
function renderWaitRoom(){
  if(!waitEl) return;
  const e=Profile.equipped;
  const isHost = window.Net && Net.amHost && Net.amHost();
  const peers = (window.Net && Net.getPeers)?Net.getPeers():[];
  const online = peers.length + (window.Net && Net.isConnected()?1:0);
  const showMap = isHost ? e.map : ((roomConfig&&roomConfig.map)||e.map);
  const showTier = isHost ? e.tier : ((roomConfig&&roomConfig.tier)||e.tier);
  let h='<div class="panel" style="width:min(560px,94vw)"><h1>房间等待室</h1>';
  h+='<div class="sub">房间码：<b style="color:#ffd86b">'+e.room+'</b> · 在线 '+online+' 人 · 房主：'+(isHost?'你':(hostName||'房主'))+'</div>';
  h+='<section><div class="stitle">行动设定'+(isHost?'（你来决定）':'（由房主决定）')+'</div>';
  h+='<div class="row"><span class="meta">地图：</span>';
  for(const m of OPS_MAPS){ h+='<button class="pill '+(showMap===m.id?'on':'')+'" '+(isHost?'':'disabled ')+'onclick="Meta.setWaitMap(\''+m.id+'\')">'+m.name+'</button>'; }
  h+='</div><div class="row"><span class="meta">难度：</span>';
  for(const k in OPS_TIERS){ const t=OPS_TIERS[k]; h+='<button class="pill '+(showTier===k?'on':'')+'" '+(isHost?'':'disabled ')+'onclick="Meta.setWaitTier(\''+k+'\')">'+t.name+'</button>'; }
  h+='</div></section>';
  h+='<section><div class="stitle">玩家列表 ('+online+')</div>';
  h+='<div class="item"><div><span class="nm" style="color:#39b6ff">'+(e.name||'你')+'（你）</span></div></div>';
  for(const p of peers){ h+='<div class="item"><div><span class="nm" style="color:'+(p.color||'#fff')+'">'+(p.name||p.id)+'</span></div></div>'; }
  h+='</section>';
  h+='<section>';
  if(isHost) h+='<button class="deploy" onclick="Meta.startRoom()">▶ 开始游戏</button>';
  else h+='<div class="meta" style="text-align:center;margin:8px 0">等待房主开始游戏…</div>';
  h+='<button class="pill" style="margin-top:8px;width:100%;text-align:center" onclick="Meta.exitRoom()">退出房间</button>';
  h+='</section></div>';
  waitEl.innerHTML=h;
}
function setWaitMap(id){ setMap(id); if(window.Net && Net.isConnected() && Net.amHost && Net.amHost()) Net.setConfig(id, Profile.equipped.tier); renderWaitRoom(); }
function setWaitTier(k){ setTier(k); if(window.Net && Net.isConnected() && Net.amHost && Net.amHost()) Net.setConfig(Profile.equipped.map, k); renderWaitRoom(); }
function exitRoom(){ if(window.Net) Net.disconnect(); hideWaitRoom(); showBase(); }
function startRoom(){
  const e=Profile.equipped;
  const seed = ((Date.now() ^ Math.floor(Math.random()*0x7fffffff)) >>> 0) || 1;
  const cfg={ weapon:e.weapon, weapon2:e.weapon2, armorLevel:e.armorLevel, meds:e.meds, ammo:e.ammo, surgery:e.surgery, keycards:carryKeycards(), insure:e.insure, map:e.map, tier:e.tier, mp:true, name:(e.name||'玩家').slice(0,12), room:e.room, seed };
  if(window.Net) Net.startRoom(cfg);
  // 房主将通过 onStart 回调进入战场（与加入者统一路径），此处不重复 startGame
}
// 收到房主开始广播（或房主本地触发）：统一进入战场
function onNetStart(shared){
  hideWaitRoom();
  normalize();
  const e=Profile.equipped;
  const cfg={ weapon:e.weapon, weapon2:e.weapon2, armorLevel:e.armorLevel, meds:e.meds, ammo:e.ammo, surgery:e.surgery, keycards:carryKeycards(), insure:e.insure, map:(shared&&shared.map)||e.map, tier:(shared&&shared.tier)||e.tier, mp:true, name:(e.name||'玩家').slice(0,12), room:e.room, seed:(shared&&shared.seed) };
  consumeAndStart(cfg);
}
function onRaidEnd(result){
  const s = Profile.stats; s.raids++;
  const rv = Math.round(Number(result.raidValue)||0); // 防止 NaN/异常数值写入存档（数据溢出修复）
  if(result.win){
    Profile.money += rv; s.extracts++; s.earned += rv;
  ...
  // 房卡数据回收：把本局带回/拾取的房卡写回仓库（避免房卡数据丢失/错乱）
  if(result.keycards){ for(const t of ['green','blue','purple','gold']){ const n=result.keycards[t]|0; if(n>0) add('keycard_'+t, n); } }
  const ex = result.win?1:0;
  for(const q of Profile.quests){
    if(q.id==='extract') q.progress += ex;
    else if(q.id==='scav'){ if(rv>=2000) q.progress += 1; }
    else if(q.id==='killer') q.progress += result.kills;
    if(q.progress>=q.goal) q.claimable=true;
  }
  save();
  // 联机对局结束：断开游戏连接（房主同时撤回大厅房间条目）
  if(Profile.equipped.mp && window.Net) Net.disconnect();
}
...
function buildDOM(){
  ...
  #base .lobby{max-height:200px;overflow-y:auto;margin:8px 0;border:1px solid #233140;border-radius:8px;padding:6px;background:#0a1016;}
  `;
...
function render(){
  ...
  h+=`<div class="row"><span class="meta">作战角色：</span>`;
  for(const id in ROLES){ const r=ROLES[id]; h+=`<button class="pill ${P.equipped.role===id?'on':''}" onclick="Meta.setRole('${id}')" style="border-left:3px solid ${r.color}">${r.name}</button>`; }
  h+=`</div>`;
  h+=`<div class="row"><span class="meta" style="color:${(ROLES[P.equipped.role]||{}).color||'#7a9'}">${(ROLES[P.equipped.role]||{}).desc||''}</span></div>`;
  // 医疗
  ...
  h+=`<div class="row"><span class="meta">医疗包(${medOk}个)：</span>
    ...
    <button onclick="Meta.step('meds',1)" ${P.equipped.meds>=Math.min(5,medOk)?'disabled':''}>+</button>
    <span class="meta">（H 键使用回血）</span></div>`;
  // 手术包
  const surOk=count('surgery');
  h+=`<div class="row"><span class="meta">手术包(${surOk}个)：</span>
    <button onclick="Meta.step('surgery',-1)">−</button>
    <span class="pill on">携带 ${P.equipped.surgery}</span>
    <button onclick="Meta.step('surgery',1)" ${P.equipped.surgery>=Math.min(5,surOk)?'disabled':''}>+</button>
    <span class="meta">（F 键清除流血/骨折）</span></div>`;
  // 房卡（自动携带全部持有，用于开启上锁密室）
  const kcNames={green:'绿卡',blue:'蓝卡',purple:'紫卡',gold:'金卡'};
  let kcLine=''; for(const t of ['green','blue','purple','gold']){ const o=count('keycard_'+t); if(o>0) kcLine+=' '+kcNames[t]+'×'+o; }
  h+=`<div class="row"><span class="meta">房卡（自动携带全部，开启密室）：</span><span class="pill on">${kcLine||'无（去黑市购买）'}</span>
    <span class="meta">（绿&lt;蓝&lt;紫&lt;金，越高越值钱，可在地图拾取）</span></div>`;
  // 弹药
  ...
  if(P.equipped.mp){
    ...
    h+=`<div class="row">
      <button class="pill" onclick="Meta.createRoom()">＋ 创建房间并部署</button>
      <button class="pill" onclick="Meta.genRoom()">随机房间码</button>
      <span class="meta">${mpStatus||''}</span></div>`;
    h+=`<div class="lobby" id="lobbyList"></div>`;
    h+=`<div class="row" style="margin-top:8px"><span class="meta">或手动加入房间码：</span>
      ...
      <button onclick="Meta.joinRoom()">加入</button></div>`;
  }
  h+=`</section>`;
...
return {
  ...
  setSlot, setArmor, setMap, setTier, setField, toggleMp, genRoom, createRoom, joinRoom,
  setRole, startRoom, exitRoom, setWaitMap, setWaitTier, onNetStart,
